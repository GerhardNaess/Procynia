...
class EnterpriseWikiDocumentFlowService
{
    ...
    public function syncDocumentOwnerApprovals(EnterpriseWikiDocument $document)
    {
        ...
        foreach ($runIds as $runId) {
            ...
            // A finished run stays finished. An owner change creates or moves a requirement row,
            // which is a change to who is recorded as responsible for the source material — not a
            // reason to reopen processing that completed, because the sign-off gates nothing.
            if (! $run->isTerminal()) {
    ...
    public function reconcileRunDocumentOwnerApprovalState(EnterpriseWikiIngestRun $run): void
    {
        ...
        // Still recorded, just no longer a gate: which source documents each generated page drew
        // on, and who owns them. This used to happen as a side effect of evaluating the gate.
        $this->documentOwnerApprovalService->syncRequirementsForRun($run);

        // A run's status describes its own processing, and the processing is done. Whether a
        // document owner has vouched for their material is recorded on those rows and decides
        // nothing, so it cannot be a reason this run is unfinished.
