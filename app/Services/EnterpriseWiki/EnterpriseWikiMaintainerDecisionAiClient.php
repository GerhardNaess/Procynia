...
class EnterpriseWikiMaintainerDecisionAiClient
{
    ...
    private function developerPrompt(string $languageName): string
    {
        return implode("\n", [
            ...
            '',
            'PAGE RESPONSIBILITY (content_responsibility / must_not_repeat / related_page_guidance):',
            '  You see every page planned for this source document at once — this is the one point',
            '  where cross-page repetition can be prevented before any page content is written.',
            '  Assign each page a distinct faglig ansvar (content_responsibility) so that no two pages',
            '  are asked to independently explain the same fact in full.',
            '  content_responsibility: 1-4 short bullet points — what THIS page, and only this page,',
            '  should explain in depth.',
            '  must_not_repeat: topics this page must not explain in full because another planned (or',
            '  existing, from the wiki index) page already owns them. Leave empty only when there is',
            '  genuinely no overlap with any other page.',
            '  related_page_guidance: for each page listed in must_not_repeat (or otherwise closely',
            '  related), name that page\'s title and a short instruction for how this page should refer',
            '  to it (e.g. "Link here for the detailed workflow — do not repeat it").',
            '  A concept/entity page that is itself the natural owner of a topic (e.g. an overarching',
            '  framework page) should have a SHORT must_not_repeat and a substantial content_responsibility',
            '  — do not shrink a page to nothing just to avoid overlap; give it real ownership instead.',
            '  The source_article and source_summary are about the same source document — give summary',
            '  a narrow content_responsibility (a short orientation and the main points) and instruct it,',
            '  via related_page_guidance, to point to the article for detail rather than repeating it.',
            '',
