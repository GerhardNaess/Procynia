    ...
    <div class="space-y-6">
        ...
        @if ($unreadCount > 0)
            <div class="flex items-center justify-between">
                ...
                <button wire:click="markAllAsRead" type="button"
                    class="rounded-xl border border-gray-200 bg-white px-3 py-1.5 text-xs font-semibold text-gray-600 shadow-sm transition hover:bg-gray-50">
                    Merk alle som lest
                </button>
            </div>
        @endif
                ...
                <div class="flex items-start justify-between gap-4">
                    <div class="flex items-start gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="mt-1 text-xs text-gray-400">{{ $n['created_at'] }}</div>
                        </div>
                    </div>
                    <button wire:click="markAsRead({{ $n['id'] }})" type="button"
                        class="shrink-0 rounded-full border border-gray-200 bg-white px-2.5 py-1 text-xs font-semibold text-gray-500 transition hover:bg-gray-50">
                        Merk lest
                    </button>
                </div>
            </div>
        @empty
            <div class="rounded-2xl border border-gray-200 bg-white px-5 py-10 text-center shadow-sm">
                <p class="text-sm text-gray-400">Ingen uleste varsler.</p>
            </div>
        @endforelse

        @if (count($recentNotifications) > 0)
            <div class="mt-6">
                <h2 class="mb-3 text-xs font-bold uppercase tracking-wider text-gray-400">Leste varsler (siste 30)</h2>
                        ...
                        <div class="px-5 py-3 text-sm text-gray-500">
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0">
                                    ...
                                    <span class="ml-2 text-gray-400">{{ $n['message'] }}</span>
                                </div>
                                <span class="shrink-0 text-xs text-gray-400">{{ $n['read_at'] }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
