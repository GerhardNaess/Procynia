<x-filament-panels::page>
    ...
    <div class="space-y-4">
        ...
        {{-- Midlertidig notice --}}
        <div class="rounded-xl border border-warning-200 bg-warning-50 p-3 text-base leading-6 text-warning-800">
        ...
        <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                ...
                <div class="rounded-xl {{ $enabled ? 'bg-success-50' : 'bg-gray-50' }} px-4 py-3">
                    <div class="text-base font-semibold uppercase tracking-[0.12em] {{ $enabled ? 'text-success-700' : 'text-gray-600' }}">
                        {{ $t('fields.backup_enabled') }}
                    </div>
                    <div class="mt-1.5 inline-flex items-center rounded-full px-2.5 py-1 text-base leading-6 font-medium {{ $enabled ? $okBadge : $errBadge }}">
                ...
                <div class="rounded-xl bg-gray-50 px-4 py-3">
                    ...
                    <div class="mt-1.5 text-base leading-6 font-medium text-gray-700">
                ...
                <div class="rounded-xl bg-gray-50 px-4 py-3">
                    ...
                    <div class="mt-1.5 text-base leading-6 font-medium text-gray-700">
                        ...
                        @else
                            <span class="text-gray-600">{{ $t('statuses.no_backup_yet') }}</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="rounded-xl bg-gray-50 px-4 py-3">
                    ...
                    <div class="mt-1.5 text-base leading-6 font-medium text-gray-700">
                        ...
                        @else
                            <span class="text-gray-600">{{ $t('statuses.not_available') }}</span>
                        @endif
                    </div>
                </div>
                        ...
                        <div class="rounded-xl border {{ $isCritical ? 'border-danger-200 bg-danger-50' : 'border-warning-200 bg-warning-50' }} p-3">
                            <div class="flex items-start gap-2.5">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="text-base leading-6 font-semibold {{ $isCritical ? 'text-danger-900' : 'text-warning-900' }}">
                                        {{ $t('warnings.'.$warningKey) }}
                                    </p>
                                    <p class="mt-1.5 text-base leading-6 {{ $isCritical ? 'text-danger-700' : 'text-warning-800' }}">
        ...
        <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            @if (empty($recentRuns))
                <p class="text-base text-gray-600">{{ $t('messages.no_runs') }}</p>
            @else
                <div class="overflow-x-auto rounded-lg border border-gray-100">
                    <table class="min-w-full divide-y divide-gray-100 text-base">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-3 py-2 text-left font-semibold uppercase tracking-[0.12em] text-gray-600 whitespace-nowrap">{{ $t('fields.type') }}</th>
                                <th class="px-3 py-2 text-left font-semibold uppercase tracking-[0.12em] text-gray-600 whitespace-nowrap">{{ $t('fields.status') }}</th>
                                <th class="hidden px-3 py-2 text-left font-semibold uppercase tracking-[0.12em] text-gray-600 whitespace-nowrap sm:table-cell">{{ $t('fields.started_at') }}</th>
                                <th class="hidden px-3 py-2 text-left font-semibold uppercase tracking-[0.12em] text-gray-600 whitespace-nowrap sm:table-cell">{{ $t('fields.finished_at') }}</th>
                                <th class="hidden px-3 py-2 text-left font-semibold uppercase tracking-[0.12em] text-gray-600 whitespace-nowrap md:table-cell">{{ $t('fields.duration') }}</th>
                                <th class="hidden px-3 py-2 text-left font-semibold uppercase tracking-[0.12em] text-gray-600 whitespace-nowrap lg:table-cell">{{ $t('fields.database_file') }}</th>
                                <th class="hidden px-3 py-2 text-left font-semibold uppercase tracking-[0.12em] text-gray-600 whitespace-nowrap lg:table-cell">{{ $t('fields.triggered_by') }}</th>
                                <th class="px-3 py-2 text-left font-semibold uppercase tracking-[0.12em] text-gray-600">{{ $t('fields.error_message') }}</th>
                            </tr>
                        </thead>
                            ...
                            @foreach ($recentRuns as $run)
                                <tr class="transition-colors hover:bg-gray-50">
                                    ...
                                    <td class="px-3 py-2 whitespace-nowrap">
                                        <span class="inline-flex items-center rounded-full px-2 py-1 text-base leading-6 font-medium {{ $statusColors[$run['status']] ?? $grayBadge }}">
                                            {{ $run['status_label'] }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-3 py-2 text-base leading-6 text-danger-700 max-w-xs truncate" title="{{ $run['error_message'] }}">
        ...
        <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            @if (! ($br['directory_exists'] ?? false))
                <p class="text-base leading-6 text-warning-800">{{ $t('messages.directory_missing') }}</p>
                <p class="mt-1 text-base leading-6 text-gray-600">{{ $br['directory'] ?? '' }}</p>
            @elseif (empty($backupFiles))
                <p class="text-base text-gray-600">{{ $t('messages.no_files') }}</p>
            @else
                <div class="overflow-x-auto rounded-lg border border-gray-100">
                    <table class="min-w-full divide-y divide-gray-100 text-base">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-3 py-2 text-left font-semibold uppercase tracking-[0.12em] text-gray-600">{{ $t('fields.filename') }}</th>
                                <th class="hidden px-3 py-2 text-right font-semibold uppercase tracking-[0.12em] text-gray-600 whitespace-nowrap sm:table-cell">{{ $t('fields.file_size') }}</th>
                                <th class="hidden px-3 py-2 text-left font-semibold uppercase tracking-[0.12em] text-gray-600 whitespace-nowrap sm:table-cell">{{ $t('fields.last_modified') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 bg-white">
                ...
                <p class="mt-2 text-base leading-6 text-gray-600">{{ $br['directory'] ?? '' }}</p>
            @endif
        </section>
        ...
        <section class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <div class="rounded-xl border border-warning-200 bg-warning-50 p-3 text-base leading-6 text-warning-800">
                ...
                <p class="mt-2 font-mono text-base text-warning-800">docs/operations/backup-restore.md</p>
            </div>
        </section>
