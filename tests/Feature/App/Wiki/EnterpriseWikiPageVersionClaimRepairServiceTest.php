...
class EnterpriseWikiPageVersionClaimRepairServiceTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_repair_skips_runs_waiting_for_owner_approval_and_leaves_data_unchanged(): void
    {
        ...
        $this->mock(EnterpriseWikiPageVersionClaimSyncService::class)->shouldNotReceive('markPageForResync');
        $this->mock(EnterpriseWikiPageVersionClaimSyncService::class)->shouldNotReceive('syncRuns');
        $this->mock(EnterpriseWikiAppliedRunLintService::class)->shouldNotReceive('lint');

        $service = app(EnterpriseWikiPageVersionClaimRepairService::class);
        $result = $service->repair($run->fresh(), true);
    ...
    public function test_repair_still_processes_an_applied_run(): void
    {
        $customer = $this->createCustomer();
        $document = $this->createDocument($customer);
        ...
        $syncService = $this->mock(EnterpriseWikiPageVersionClaimSyncService::class);
        $syncService
            ->shouldReceive('markPageForResync')
            ->once()
            ->withArgs(fn (EnterpriseWikiPage $givenPage): bool => $givenPage->is($page))
            ->andReturn([$run->id]);

        $syncService
            ->shouldReceive('syncRuns')
            ->once()
            ->with([$run->id]);
        ...
        $this->assertSame(1, $result['runs_checked']);
        $this->assertSame(1, $result['pages_checked']);
        $this->assertSame(1, $result['pages_missing_claims']);
        $this->assertSame(0, $result['pages_already_synced']);
    }

    private function createCustomer(string $name = 'Claim Repair Guard Test AS'): Customer
    {
    ...
    private function createDocument(Customer $customer): EnterpriseWikiDocument
    {
        return EnterpriseWikiDocument::query()->create([
            ...
            'extracted_text' => 'Source document text for claim repair guard tests.',
