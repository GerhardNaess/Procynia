...
class CustomerSavedNoticeWorklistTest extends TestCase
{
    ...
    public function test_customer_can_update_history_metadata_for_own_archived_notice(): void
    {
        $context = $this->customerAdminContext();
        $awardDate = now()->startOfDay();
        $expectedNextProcessDate = $awardDate->copy()->addMonthsNoOverflow(48)->subMonthsNoOverflow(6);
        $savedNotice = $this->createSavedNotice(
            $context['customer']->id,
            '2026-1002-h',
            'Historikkpost',
            archived: true,
            awardDateAt: $awardDate->toDateTimeString(),
            status: null,
        );
        ...
        $response = $this->actingAs($context['admin'])
            ...
            ->patch("/app/notices/saved/{$savedNotice->id}/history-metadata", [
                ...
                'next_process_date_at' => now()->addYears(20)->toDateString(),
            ]);
    ...
    public function test_history_payload_includes_history_metadata_fields(): void
    {
        $context = $this->customerAdminContext();
        $savedNotice = $this->createSavedNotice(
            $context['customer']->id,
            ...
            nextProcessDateAt: now()->addMonths(14)->startOfDay()->toDateTimeString(),
        );
    ...
    public function test_history_metadata_update_sets_next_process_date_to_null_when_award_or_period_is_missing(): void
    {
        $context = $this->customerAdminContext();
        $missingAward = $this->createSavedNotice($context['customer']->id, '2026-1002-null-a', 'Mangler award', archived: true, status: null);
        $missingPeriod = $this->createSavedNotice(
            $context['customer']->id,
            '2026-1002-null-p',
            'Mangler periode',
            archived: true,
            awardDateAt: now()->startOfDay()->toDateTimeString(),
            status: null,
        );
        ...
        $this->actingAs($context['admin'])
            ->withSession(['_token' => 'test-token'])
            ->withHeaders(['X-CSRF-TOKEN' => 'test-token'])
            ->patch("/app/notices/saved/{$missingAward->id}/history-metadata", [
                'contract_period_months' => 48,
            ])
            ->assertRedirect();
        ...
        $this->actingAs($context['admin'])
            ->withSession(['_token' => 'test-token'])
            ->withHeaders(['X-CSRF-TOKEN' => 'test-token'])
            ->patch("/app/notices/saved/{$missingPeriod->id}/history-metadata", [
                'contract_value_mnok' => 7.5,
            ])
            ->assertRedirect();

        $this->assertNull($missingAward->fresh()->next_process_date_at);
        $this->assertNull($missingPeriod->fresh()->next_process_date_at);
    }

    public function test_history_metadata_update_validates_structured_numeric_fields(): void
    {
        $context = $this->customerAdminContext();
        $savedNotice = $this->createSavedNotice($context['customer']->id, '2026-1002-invalid', 'Ugyldig historikk', archived: true, status: null);
        ...
        $this->assertNull($savedNotice->contract_value_mnok);
    ...
    public function test_store_saved_notice_persists_saved_by_user_id_for_first_save(): void
    {
        $context = $this->customerAdminContext();
    ...
    public function test_store_saved_notice_does_not_overwrite_existing_saved_by_user_id(): void
    {
        ...
        $response = $this->actingAs($original['admin'])
            ->withSession(['_token' => 'test-token'])
            ->withHeaders(['X-CSRF-TOKEN' => 'test-token'])
