...
class OperationalDeviationResourceTest extends TestCase
{
    ...
    public function test_seeder_creates_all_thirty_deviations_on_fresh_database(): void
    {
        ...
        // AVVIK-023 must be closed (Playwright E2E-oppsett innført)
        $avvik023 = OperationalDeviation::query()->where('code', 'AVVIK-023')->firstOrFail();
        $this->assertSame(OperationalDeviation::STATUS_CLOSED, $avvik023->status);
        $this->assertNotNull($avvik023->closed_at);
        $this->assertNotNull($avvik023->verified_at);

        // AVVIK-017 must be closed (brukerprompt per krav implementert)
        $avvik017 = OperationalDeviation::query()->where('code', 'AVVIK-017')->firstOrFail();
        $this->assertSame(OperationalDeviation::STATUS_CLOSED, $avvik017->status);
        $this->assertNotNull($avvik017->closed_at);
        $this->assertNotNull($avvik017->verified_at);

        // AVVIK-026 must be closed (intern admin-guide etablert)
        $avvik026 = OperationalDeviation::query()->where('code', 'AVVIK-026')->firstOrFail();
        $this->assertSame(OperationalDeviation::STATUS_CLOSED, $avvik026->status);
