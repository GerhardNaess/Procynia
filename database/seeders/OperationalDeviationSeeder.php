...
class OperationalDeviationSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $this->seed([
            ...
            'description' => 'Kostnadskontrollen er ferdig implementert og verifisert i Fase 1-6: måling, commercial quota, atomiske reservasjoner, kill switches, NOK-sikkerhetsbudsjett, pris-/FX-policy, betalingsstatus, adminflate og revisjonsspor. Gjenstår kun produksjonsaktivering: migrasjonene er ikke kjørt i noe kjørende miljø, og ops:runtime-check feiler derfor på manglende kostnadskontroll-skjema.',
            ...
            'verified_at' => null,
            'closed_at' => null,
            'verification_notes' => 'AVVIKET forblir åpent på ett gjenstående punkt: produksjonsaktivering. Fase 6 verifiserte hele kjeden mot faktisk kode og test. Alle 16 closure-kriterier er PASS: eneste providerutgang er OpenAiClient, og alle tre kostnadsbærende metoder autoriserer; get() er kun det gratis /models-kallet for helsesjekk. Kontekst settes i 17 køjobber, 5 operatørkommandoer og den schedulerte QA-kommandoen, ingen kjente uklassifiserte produksjonsstier. Concurrency er bevist med ekte parallelle prosesser i begge lag: 1 credit / 10 samtidige gir 1 tildelt og 1 committed usage; NOK-budsjett for 2 / 10 samtidige gir noyaktig 2 uten overskridelse, ogsa for en unlimited-plan. Global manuell stopp og globale NOK-budsjett kan ikke omgas av operatoroverstyring (PLATFORM_REASONS). Ukjent modellpris blir aldri 0 kr. Fase 6 la i tillegg deploy-blokkerende preflight-sjekker og en timesbasert helsesveip for usikre hold, samt driftsrunbook. Gjenstar for closure: kjor migrasjonene i malmiljoet og fa ops:runtime-check gronn. Per na er alle kostnadskontroll-migrasjoner Pending i det eneste kjorende miljoet, og guarden vil da feile med databasefeil i stedet for kontrollert stopp.',
        ]);
